...
public final class WallpaperList {
    public static final String WALLPAPER_CATEGORY[] = {
            "Abstract",
            "Animals",
            "Entertainment",
            "Holidays",
    ...
    private static List<Wallpaper> list;
    private static long count = 0;

    public static List<Wallpaper> getList() {
        if (list == null) {
            list = setupWallpapers();
        }
        return list;
    }

    public static List<Wallpaper> setupWallpapers() {
        list = new ArrayList<>();

        String[][] categoryWallpapers = {
                {
                        "Night Lake",
                        "Moonshine Ocean",
                        "Red Circle",
        ...
        int bgIndex = 0; // Initialize background image index

        for (int categoryIndex = 0; categoryIndex < categoryWallpapers.length; categoryIndex++) {
            String[] categoryImages = categoryWallpapers[categoryIndex];
            String categoryTitle = WALLPAPER_CATEGORY[categoryIndex];

            for (int index = 0; index < categoryImages.length; index++) {
                String bgImageUrl = bgImageUrls[bgIndex % bgImageUrls.length];
    ...
    private static Wallpaper buildWallpaperInfo(
            String title,
            //String description,
           // String studio,
           // String videoUrl,
        ...
        wallpaper.setId(count++);
        wallpaper.setTitle(title);
        //wallpaper.setDescription(description);
        //wallpaper.setStudio(studio);
        wallpaper.setCardImageUrl(cardImageUrl);
        wallpaper.setBackgroundImageUrl(backgroundImageUrl);
       // wallpaper.setVideoUrl(videoUrl);
        return wallpaper;
    }
}